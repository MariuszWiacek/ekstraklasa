...
const Stats = () => {
  ...
  const [userMostChosenScores, setUserMostChosenScores] = useState({});
  ...
  useEffect(() => {
    ...
    const userScoreCounts = {}; // Track the scores chosen by users
      ...
      for (const kolejkaId in kolejkaPoints) {
        ...
        kolejekBets.forEach((bet) => {
          ...
          if (result) {
            ...
            if (!teamChoices[team]) {
              teamChoices[team] = 0;
              teamCorrectChoices[team] = 0;
              teamFails[team] = 0;
            }

            teamChoices[team]++;

            if (bet.bet === (homeScore === awayScore ? 'X' : homeScore > awayScore ? '1' : '2')) {
              teamCorrectChoices[team]++;
            }

            if ((bet.bet === '1' && homeScore < awayScore) ||
                (bet.bet === '2' && awayScore < homeScore) ||
                (bet.bet === 'X' && homeScore !== awayScore)) {
              teamFails[team]++;
            }
          }
        });
    ...
    // Find the most chosen score for each user
    const mostChosenScores = Object.keys(userScoreCounts).map((user) => {
      const scores = userScoreCounts[user];
      const mostChosenScore = Object.keys(scores).reduce((a, b) => scores[a] > scores[b] ? a : b, null);
      return { user, mostChosenScore };
    });

    const mostChosenScoresData = mostChosenScores.reduce((acc, { user, mostChosenScore }) => {
      acc[user] = mostChosenScore;
      return acc;
    }, {});

    setUserMostChosenScores(mostChosenScoresData);
      ...
      <Row>
        <Col md={12}>
          ...
          <div style={averagePointsStyle}>
            <h4>Średnie punkty</h4>
            <ul>
              {averagePoints.map((entry) => (
                <li key={entry.user}>
                  {entry.user}: {entry.value}
                </li>
              ))}
            </ul>
          </div>
          ...
          <div style={averagePointsStyle}>
            <h4>Najwięcej wybranych niepoprawnych drużyn</h4>
            <p>Najwięcej wybranych niepoprawnych drużyn: {mostChosenFailedTeam?.team || 'Brak'}</p>
          </div>

          <div style={averagePointsStyle}>
            <h4>Najwięcej poprawnych drużyn</h4>
            <p>Najwięcej poprawnych drużyn: {mostChosenCorrectTeam?.team || 'Brak'}</p>
          </div>
